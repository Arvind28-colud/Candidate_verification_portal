...
logger = logging.getLogger("import_single_candidate")
...
def parse_mysql_url(url_str: str):
    ...
    try:
        ...
        logger.info(f"Connected to Target DB: {db.DB_HOST}:{db.DB_PORT}/{db.DB_NAME} (User: {db.DB_USER})")
    except Exception as e:
        logger.warning(f"Could not parse MySQL URL: {e}")

def restore_candidate_id(target_id: str, json_filepath: str = "candidates_backup.json", mysql_url: str = None):
    """Restores/updates a SINGLE candidate record and all Base64 images by candidate_id."""
    if mysql_url:
    ...
    try:
        ...
        if not target_cand:
            logger.error(f"Candidate '{target_id}' not found in {json_filepath}.")
            return
        ...
        logger.info(f"Restoring Candidate '{cid}' ({c.get('full_name')}). Base64 lengths: face={len(face_pic)}, vault={len(vault_pic)}, front={len(front_pic)}, back={len(back_pic)}")
        ...
        if existing:
            ...
            execute_query(update_sql, (
                ...
                face_pic, front_pic, back_pic, vault_pic,
            ...
            logger.info(f"✅ Candidate '{cid}' successfully updated with all Base64 photos!")
        else:
            insert_sql = """
            ...
            logger.info(f"✅ Candidate '{cid}' successfully inserted with all Base64 photos!")

    except Exception as e:
        logger.error(f"Failed to restore candidate '{target_id}': {e}")

if __name__ == "__main__":
    target_id = sys.argv[1] if len(sys.argv) > 1 else "ID0001"
    json_path = sys.argv[2] if len(sys.argv) > 2 else "candidates_backup.json"
    url = sys.argv[3] if len(sys.argv) > 3 else None
    restore_candidate_id(target_id, json_filepath=json_path, mysql_url=url)
